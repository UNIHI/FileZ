...
abstract class Fz_Db_Table_Row_Abstract {
  ...
  public function __call ($method, $args) {
    ...
    if (substr ($method, 0, 3) == 'get') {
      ...
      if ($var == 'is_admin') {
        // XXX security concern: always remove admin privileges
        return 0;
      } else {
        if (!is_array($args) || count($args) == 0) {
          fz_log (
            "Error, trying to set variable value without supplying a value",
            FZ_LOG_ERROR);
          halt (SERVER_ERROR, 'Internal server error');
          exit;
        } else {
          return $this->$var = $args[0];
        }
      }
    } else {
